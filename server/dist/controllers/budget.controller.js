 ...
 */
export const createBudget = catchErrors(async (req, res) => {
    const userId = req.userId;
    ...
    if (existing) {
        throw new AppError('Budget for this category already exists', HTTP_STATUS.CONFLICT, 'DUPLICATE_ERROR');
    }
    // Create budget
...
export const updateBudget = catchErrors(async (req, res) => {
    ...
    const { id } = req.params;
    const { category, maximum, theme } = req.body;
    const budget = await Budget.findOneAndUpdate({ _id: id, userId }, {
        ...(category && { category }),
        ...(maximum && { maximum }),
        ...(theme && { theme }),
