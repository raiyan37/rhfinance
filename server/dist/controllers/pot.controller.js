...
// GET ALL POTS
// =============================================================================
export const getPots = catchErrors(async (req, res) => {
...
// GET SINGLE POT
// =============================================================================
export const getPot = catchErrors(async (req, res) => {
    const userId = req.userId;
    const { id } = req.params;
 ...
 */
export const createPot = catchErrors(async (req, res) => {
    const userId = req.userId;
...
// UPDATE POT
// =============================================================================
export const updatePot = catchErrors(async (req, res) => {
    const userId = req.userId;
    const { id } = req.params;
    const { name, target, theme } = req.body;
    const pot = await Pot.findOneAndUpdate({ _id: id, userId }, {
        ...(name && { name }),
        ...(target && { target }),
        ...(theme && { theme }),
 ...
 */
export const deletePot = catchErrors(async (req, res) => {
    const userId = req.userId;
    const { id } = req.params;
 ...
 */
export const depositToPot = catchErrors(async (req, res) => {
    const userId = req.userId;
    const { id } = req.params;
    const { amount } = req.body;
    // Validation
    if (!amount || amount <= 0) {
        throw new AppError('Amount must be a positive number', HTTP_STATUS.BAD_REQUEST, 'VALIDATION_ERROR');
    }
    ...
    if (user.balance < amount) {
        throw new AppError('Insufficient balance', HTTP_STATUS.BAD_REQUEST, 'INSUFFICIENT_BALANCE');
    }
    // Find pot
    const pot = await Pot.findOne({ _id: id, userId });
    if (!pot) {
 ...
 */
export const withdrawFromPot = catchErrors(async (req, res) => {
    const userId = req.userId;
    const { id } = req.params;
    ...
    if (pot.total < amount) {
        throw new AppError('Insufficient pot balance', HTTP_STATUS.BAD_REQUEST, 'INSUFFICIENT_POT_BALANCE');
    }
    // Update pot total
    pot.total -= amount;
    await pot.save();
